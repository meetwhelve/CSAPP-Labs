...
int isLessOrEqual(int x, int y) {
    int signX = (x >> 31) & 1;
    int signY = (y >> 31) & 1;
    int sign_XsubY = ((y + ~x + 1) >> 31) & 1;
    return (signX & ~signY) | (!(signX ^ signY) & !sign_XsubY);
}
//4
/* 
...
int howManyBits(int x) {
    ...
    x = (flag & (~x)) | ((~flag) & x);
    bit_16 = (!!(x >> 16)) << 4;    //  (!((!!(x >> 16)) ^ (0x01))) << 4;
    x >>= bit_16;
    bit_8  = (!!(x >> 8)) << 3;     //  (!((!!(x >> 8)) ^ (0x01))) << 3;
    x >>= bit_8;
    bit_4  = (!!(x >> 4)) << 2;     //  (!((!!(x >> 4)) ^ (0x01))) << 2;
    x >>= bit_4;
    bit_2  = (!!(x >> 2)) << 1;     //  (!((!!(x >> 2)) ^ (0x01))) << 1;
    x >>= bit_2;
    bit_1  = (!!(x >> 1));          //  (!((!!(x >> 1)) ^ (0x01)));
    x >>= bit_1;
    bit_0  = x;
...
int floatFloat2Int(unsigned uf) {
    unsigned s = (uf >> 31) & 0x1;
    unsigned expr = (uf >> 23) & 0xFF; int E = expr - 127;
    unsigned frac = uf & 0x7FFFFF;
